...
###  2. 스프링 배치 기능 추가에 따른 회원관리 기능 개선
스프링 배치 기능 도입 예정에 따른( 가입 후 1년 지난 회원 등급 자동 업그레이드 배치 )기반 작업
으로 기존 회원 관리 로직과 DB 구조 수정
...
#### 1. DB변경
- 기존 member_table 테이블에 join_date, grade 칼럼 추가

| 필드명              | 타입            | 널 허용 | 키   | 설명               |
| ----------------- | -------------- | ---- | --- | ---------------- |
| `join_date`       | `Date` | X    |    | 가입일         |
| `grade`           | `VARCHAR(255)` | X    |     | 등급             |
<br>

#### 2. 백엔드 수정
- 회원 엔티티(Entity)에 joinDate, grade 필드 추가 및 매핑
- DTO에 신규 필드 추가
